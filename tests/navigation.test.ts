...
describe("Content & LinkedIn navigation organization", () => {
  ...
  it("keeps the Development Console visibly disabled with no action route", () => {
    expect(consoleScaffold).toContain("Execution is disabled");
    expect(consoleScaffold).toContain("fieldset disabled");
    expect(consoleScaffold).not.toContain("method=\"post\"");
  });
