...
export async function loader({ context, request }: Route.LoaderArgs) {
  ...
  const [runs, approvals, summary] = await Promise.all([listAgentRuns(env.linkedinadam_db), listApprovals(env.linkedinadam_db), agentControlSummary(env.linkedinadam_db)]);
  return { user, agents:AGENT_CATALOG, runs, approvals, summary:{...summary,activeAgents:AGENT_CATALOG.filter(a=>a.status === "active").length} };
}
