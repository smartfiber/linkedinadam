...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (["generate_summary","generate_prompt","generate_follow_up","add_response","report_failure","set_work_state","edit_prompt","mark_prompt_sent","add_attachments","archive_request","restore_request","hard_delete"].includes(intent)) {
    ...
    try {
      if (intent === "generate_summary") { const copilot = await getCopilotContext(env.linkedinadam_db, requestId); await generateSummary(env,user,record,copilot); return {ok:true,message:"Summary and technical interpretation generated."}; }
